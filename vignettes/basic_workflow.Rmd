...
1.  Load the `demo`, `drug`, and `mp_short` tables.
2.  Select one or more medications of interest.
3.  Identify the drug codes (e.g., the `DrecNo(s)`)
associated with these drug using the `mp` (or `mp_short`)
table.
...
The function `get_drecno()` allows you to query the `mp_short`
...
correspondence between the name and the code (here, `mp_short`).
...
We see that `get_drecno()` finds two entries containing the drug "nivolumab" in the `mp_short` table:
...
correspondence with `mp_short` to find `DrecNo`.
...
The `get_atc_code()` function requires the `mp_short` 
