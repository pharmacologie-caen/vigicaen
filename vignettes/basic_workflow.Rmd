...
Ici, nous avons regardé dans `demo` combien de lignes nivolumab avaient la valeur 1, c'est à dire combien de cas ont été identifiés comme rapportant du nivolumab. Au total, 36,729 cas rapportent du nivolumab.
...
demo <- 
  demo %>% 
  mutate(death = 
           fifelse(UMCReportId %in% out[, UMCReportId],
...
A l'aide de conditions dans demo, trouver a, b, c, et d et assigner les résultats à des objets de même nom. La fonction `demo %>% summarise(n())` permet de compter le nombre de lignes dans demo. La fonction `filter()` permet de sélectionner des lignes selon certaines conditions. Pour passer du format data.frame au format vecteur, on utilise la fonction `pull()`.

#### Réponse

A l'aide de conditions dans demo, trouver a, b, c, et d et assigner les résultats à des objets de même nom. La fonction `demo %>% summarise(n())` permet de compter le nombre de lignes dans demo. La fonction `filter` permet de sélectionner des lignes selon certaines conditions.
...
```{r}
mod2 <- glm(colitis ~ nivolumab + sex,
            data = demo,
            family = "binomial")
...
```{r}
mod3 <- glm(colitis ~ ipilimumab + sex,
            data = demo,
            family = "binomial")
...
```{r echo=FALSE}
ror_ipi <- compute_or_mod(summary(mod3)$coefficients)[rn == "ipilimumab", orl]
ror_sex <- compute_or_mod(summary(mod3)$coefficients)[rn == "sex", orl]
```
...
```{r}
mod4 <- glm(colitis ~ ipilimumab + sex + ipilimumab * sex,
            data = demo,
            family = "binomial")
