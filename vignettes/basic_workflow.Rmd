...
(DrecNos) most of the time, or Record Ids (Record_Id) in some
specific scenarii.
...
`Record_Id`. It is therefore necessary to make a second
...
an ATC class. It is possible to retrieve Record_Ids
instead by setting the `vigilyze` argument to `FALSE`.

The interest of using Record_Ids instead of DrecNos is to
...
-  `add_serious()` and `add_death()` handle both in-memory and
out-of-memory (Arrow) tables transparently.
