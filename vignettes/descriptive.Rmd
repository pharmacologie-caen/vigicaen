...
This vignette uses the preloaded datasets.
...
See vignette \code{vignette("basic_workflow", package = "vigicaen")}
for more details on adding drug and adr columns.
...
The `desc_facvar` function allows to generate a summary of categorical
variables with 2 or more levels.

It takes as argument a dataset to describe, and one or more variables of
interest, in the `vf` argument, as a character vector.
...
The error message "too many levels detected in year, see details" is
...
final table of a manuscript. A categorization of the reporting years is
therefore desirable.
...
```{r screen_drug}
screen_drug(drug_, mp_data = mp_, top_n = 5)
```
...
setting the `data_type` argument to "adr".
...
The `data_type` argument should be set to "link".
...
This information, that we call `tto_max`, is accessible with the
`extract_tto` function.
...
notably with the `desc_tto()` function.
...
Several drugs and reactions can be quiried in these two functions.
...
The `desc_dch()` function synthesizes the number of positive dechallenges:
