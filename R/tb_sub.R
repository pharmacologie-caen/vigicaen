...
AgeGroup <-
  AgeGroup |>
  dplyr::transmute(
    AgeGroup = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Dechallenge <-
  Dechallenge |>
  dplyr::transmute(
    Dechallenge1 = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Dechallenge2 <-
  Dechallenge2 |>
  dplyr::transmute(
    Dechallenge2 = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Frequency <-
  Frequency |>
  dplyr::transmute(
    FrequencyU = substr(f0, start=1, stop=3),
    Code = substr(f0, start=4, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Gender <-
  Gender |>
  dplyr::transmute(
    Gender = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Notifier <-
  Notifier |>
  dplyr::transmute(
    Type = substr(f0, start=1, stop=2),
    Code = substr(f0, start=3, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Outcome <-
  Outcome |>
  dplyr::transmute(
    Outcome = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Rechallenge <-
  Rechallenge |>
  dplyr::transmute(
    Rechallenge1 = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Rechallenge2 <-
  Rechallenge2 |>
  dplyr::transmute(
    Rechallenge2 = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Region <-
  Region |>
  dplyr::transmute(
    Region = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
RepBasis <-
  RepBasis |>
  dplyr::transmute(
    Basis = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
ReportType <-
  ReportType |>
  dplyr::transmute(
    ReportType = substr(f0, start=1, stop=1),
    Code = substr(f0, start=2, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
RouteOfAdm <-
  RouteOfAdm |>
  dplyr::transmute(
    Route = substr(f0, start=1, stop=2),
    Code = substr(f0, start=3, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
Seriousness <-
  Seriousness |>
  dplyr::transmute(
    Seriousness = substr(f0, start=1, stop=2),
    Code = substr(f0, start=3, stop=nchar(f0))
  ) |>
  dplyr::compute()
...
SizeUnit <-
  SizeUnit |>
  dplyr::transmute(
    AmountU = substr(f0, start=1, stop=2),
    Code = substr(f0, start=3, stop=nchar(f0))
  ) |>
  dplyr::compute()
